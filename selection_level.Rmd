---
title: "Do the different type of selection work?"
output:
...
###Checking all intermediate regime
#### soft-metaPop on metapop side
```{r}
my_sim@sim.name <- "soft_metapop0"
my_sim  <- setParameter(my_sim,"selection_level",1)
my_sim  <- setParameter(my_sim,"selection_level_coef",1)  
result <- run(my_sim, verbose=F)
stat <- loadStat(my_sim)
stopifnot(stat$adlt.nbInd_p1 == statMetaPop$adlt.nbInd_p1)
stopifnot(stat$adlt.nbInd_p2 == statMetaPop$adlt.nbInd_p2)
```

#### soft-metaPop on soft side
does not work, this is why we commented the test
```{r}
my_sim@sim.name <- "soft_metapop0"
my_sim  <- setParameter(my_sim,"selection_level",1)
my_sim  <- setParameter(my_sim,"selection_level_coef",0)  
result <- run(my_sim, verbose=F)
stat <- loadStat(my_sim)
#stopifnot(stat$adlt.nbInd_p1 == statSoft$adlt.nbInd_p1)
#stopifnot(stat$adlt.nbInd_p2 == statSoft$adlt.nbInd_p2)
```

#### soft-metaPop in the middle
```{r}
my_sim@sim.name <- "soft_metapop0"
my_sim  <- setParameter(my_sim,"selection_level",1)
my_sim  <- setParameter(my_sim,"selection_level_coef",1)  
