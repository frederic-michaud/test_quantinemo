...
plot(pop,xlim = c(0,generations),ylim = c(5,10*grow.rate^generations),log = "y", t = "l")
```
...
```{r}
n.replicate = 40
pops = matrix(0, nrow = n.replicate, ncol = generations)
...
pops.final = pops[, generations]
#both test should fail about 1/40 (1/n.replicate) i.e 2.5 % of the time. We have therefore 5% chance to fail the test. 
stopifnot(min(pops.final) < pop.sim)
stopifnot(max(pops.final) > pop.sim)
```
