...
stopifnot(sd(phenotype.1s) > 1.3)
stopifnot(sd(phenotype.1s) < 1.5)
```
...
sd(phenotype.2s)
stopifnot(sd(phenotype.2s) > 0.9)
stopifnot(sd(phenotype.2s) < 1.1)
```
```{r}
length(phenotype.0.5p)
```
...
stopifnot(sd(phenotype.0.5p) > 0.18)
stopifnot(sd(phenotype.0.5p) < 0.22)
```
